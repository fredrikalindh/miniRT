...
/*   Updated: 2020/01/11 16:29:56 by frlindh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	hit_sp(t_intersection *i, t_ray ray, void *shape)
{
	...
	if ((!(t > T_MIN && t < T_MAX) || t > i->t) &&
	(t = (-b + sqrt(sqr(b) - 4 * a * c)) / (2 * a)) != 0.0;
		a = -1;
	if (!(t > T_MIN && t < T_MAX && t < i->t && t != 0.0))
		return (0);
	i->shape = (void *)sphere;
	...
	i->color = (uv) ? get_uv(0.5 + (atan2(i->normal.z, i->normal.x) * 0.159159),
	0.5 - (sin(i->normal.y) * 0.31832), pattern) : sphere->color;
	i->uv.x = 0.5 + (atan2(i->normal.z, i->normal.x) * 0.159159);
	i->uv.y = 0.5 - (sin(i->normal.y) * 0.31832);
	i->color = (((int)(i->uv.x * sphere->radius * 6) % 2 == 0 && (int)(i->uv.y * sphere->radius * 4) % 2 != 0) || ((int)(i->uv.x * sphere->radius * 6) % 2 != 0 && (int)(i->uv.y * sphere->radius * 4) % 2 == 0)) ? new_color(255 - sphere->color.r, 255 - sphere->color.g, 255 - sphere->color.b) : sphere->color;
	return (1);
}
